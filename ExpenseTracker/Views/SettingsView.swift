...
struct SettingsView: View {
        ...
        let rows = transactions
            ...
            .map {
                ...
                return [
                    ...
                    $0.transferID?.uuidString ?? "", $0.tags.joined(separator: ", "),
    ...
    private func backupFingerprint(_ transaction: Transaction) -> String {
        backupFingerprint(amount: transaction.amount, currency: transaction.currencyCode ?? currencyCode,
                          ...
                          notes: transaction.notes, transferID: transaction.transferID, tags: transaction.tags, createdAt: transaction.createdAt)
    }
    ...
    private func backupFingerprint(amount: Double, currency: String, type: TransactionType, categoryRaw: String, payment: PaymentMethod, transactionDate: Date, merchant: String, notes: String, transferID: UUID?, tags: [String], createdAt: Date) -> String {
        DomainLogic.csv(rows: [[String(amount), currency, type.rawValue, categoryRaw, payment.rawValue,
                                transactionDate.ISO8601Format(), merchant, notes, transferID?.uuidString ?? "", tags.joined(separator: ", "), createdAt.ISO8601Format()]])
    }
